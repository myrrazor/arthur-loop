...
The three commands above track default git HEAD (usually `main`). The last tag is **v0.1.0** — there is no tagged v0.2.0. `arthur --version` prints `arthur 0.1.0` until the next tag. That public tree is a **file cockpit**: `init`, queue, capture, gate, `status`, `arthur web`. It does **not** ship `arthur follow`, the MCP server, or Atlas walk, and it is not a working auto-follow in minutes. `install.ps1` is not on public main (that URL 404s). Canonical site: [arthurloop.com](https://arthurloop.com/) (`arthur-loop.vercel.app` 404s).
...
- **Human decisions** — `arthur decision open|answer|clear|list`. A blocking question pauses *its* project, never the others. Pause refuses claim, submit, poll-result, complete, and fail (exit 2).
...
**Trackers.** For [Atlas Tasker](https://github.com/myrrazor/atlas-tasker) the primary path is the board: `arthur tracker board --json` / MCP `arthur.board` reads `tracker board --json`, and `arthur tracker open-jobs` opens queue jobs from ready/assigned tickets (`atlas:<ticket_id>` idempotency keys). Arthur `project_id` maps to an Atlas `--project` key via `tracker.project_map`. `arthur init --project-id MY_APP --tracker atlas-tasker` and `arthur loop create` write `MY_APP` → `MY`. An empty map after init-with-no-projects is printed, not silent — fill it before `arthur tracker walk`. The three argv templates (`open_decision`, `close_decision`, `sprint_gate`) remain as a fallback — rendered to argv, never a shell, run from the instance root. Gate logic still lives in the artifact store, not the tracker. `arthur decision open` still calls `open_decision`. Any other CLI tracker works with your own templates. Or pick `none` and decisions live in `human-decisions/open.md` alone.
